...
> Last updated: 2026-06-11 (DEC-0015 added)
...
Use the format below. Do not delete old decisions — mark them as `Superseded` or `Deprecated` instead.
...
- SAS tokens expire (1 hour) and grant only write permission to a specific sample, limiting security exposure.
...
**Context:**  
During stack bringup, the host port 8000 was already occupied by the container `nodepay-ai-service-1`, belonging to another project in the same development environment.

**Decision:**  
Map the host port of the API to `8001` (`8001:8000` in `docker-compose.yml`). The internal port of the container remains `8000` (Uvicorn continues to run on `0.0.0.0:8000` inside the container).

**Reason:**  
- The alternative of stopping `nodepay-ai-service` would interrupt another active project.
- The port change is local to Daniel's environment; the internal port remains unchanged, so inter-container communication (worker -> API) is not affected.

**Impact:**  
- `frontend/.env`: `VITE_API_BASE_URL=http://localhost:8001/api/v1`
- `frontend/src/services/api.ts` and `useJobStream.ts`: hardcoded fallback updated to `localhost:8001`.
- Swagger UI accessible at `http://localhost:8001/docs`.
- If in the future port 8000 becomes free, revert to `8000:8000` in `docker-compose.yml` and update the three frontend files.
...
**Context:**  
The video rendering process (`workers.py`) using FFmpeg was very slow because the default values of the `libx264` encoder (preset `medium`) were used. Additionally, `ValueError` crashes occurred when reading `N/A` values from the FFmpeg progress log.

**Decision:**  
FFmpeg was explicitly configured to use `-c:v libx264`, `-preset ultrafast`, `-crf 23`, and `-threads 0`. A defensive `try/except` block was also added to ignore non-numeric values (`N/A`) emitted in the `out_time_ms` stream.

**Reason:**  
- `ultrafast` dramatically reduces rendering time (up to 50-70% faster), utilizing all available threads (`-threads 0`).
- The increase in file size is acceptable for the MVP nature of the project.
- Prevents the Celery task from failing prematurely due to progress parsing errors.

**Impact:**  
- Better perception of rendering performance.
- It is not possible to use `-c:v copy` since the process applies video filters (`-vf`) for burned-in subtitles and brightness equalization.
...
**Context:**  
`blob_client.upload_blob(data, overwrite=True)` with no configuration uses the Azure SDK's default socket write timeout of ~20 seconds. A ~21.7 MB rendered MP4 uploaded from inside a Docker container (which routes through the host network adapter) was consistently timing out, triggering the SDK's internal retry loop (3 retries × 20s = 60+ seconds blocked) before failing with `ServiceResponseError: ("Connection aborted.", TimeoutError("The write operation timed out"))`.

The 10+ minute job duration was caused by this retry cycle combined with no Celery task time limit, leaving the worker blocked indefinitely.
...
**Reason:**  
- Block upload means a 21.7 MB file is split into ~6 blocks of 4 MB. If any block's connection drops, only that block retries — not the entire file.
- `read_timeout=300` gives each block 5 minutes to complete, handling legitimate network slowness without failing the entire job.
...
- `app/infrastructure/workers.py`: Added `SoftTimeLimitExceeded` handler to both tasks so that temp files are cleaned up and the job is marked `FAILED` gracefully before the hard kill fires.
- All other upload/download/SAS behavior is unchanged.
- If the SDK version is upgraded beyond `azure-storage-blob==12.28.0`, verify that the `max_single_put_size`, `max_block_size`, `connection_timeout`, and `read_timeout` kwargs are still accepted by `BlobServiceClient.from_connection_string()`.
